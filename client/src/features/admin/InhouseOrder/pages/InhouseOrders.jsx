  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-700 font-sans text-black p-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative group">
            ...
            <input 
              ...
              className="pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold outline-none focus:ring-4 focus:ring-[#7e2827]/5 w-64 transition-all"
            />
          </div>
          ...
          <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 p-1.5 rounded-xl shadow-inner">
            ...
            <DatePicker 
              ...
              className="w-24 bg-transparent text-[10px] font-black uppercase outline-none text-slate-600"
            />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatusCard label="Current Page Orders" value={stats.total} icon={<ArrowUpRight size={16}/>} color="black" />
        <StatusCard label="Active Prep" value={stats.pending} icon={<Timer size={16}/>} color="amber" />
        <StatusCard label="Delivered Items" value={stats.completed} icon={<PackageCheck size={16}/>} color="emerald" />
        <StatusCard label="Issues/Cancelled" value={stats.cancelled} icon={<XCircle size={16}/>} color="rose" />
      </div>
      ...
      <div className="bg-white p-2 rounded-[3rem] border border-slate-200 shadow-sm overflow-hidden min-h-[400px]">
        {loading ? (
          <div className="flex flex-col items-center justify-center h-96 space-y-4">
            ...
            <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.3em]">Syncing Fleet...</p>
          </div>
          ...
          <div className="flex flex-col justify-center items-center h-96 text-slate-300">
            ...
            <p className="font-black uppercase text-[10px] tracking-widest">No orders found in this period</p>
          </div>
        ) : (
      ...
      {/* 4. Modern Pagination Area */}
      <div className="flex justify-between items-center px-6 py-4 bg-white rounded-2xl border border-slate-200 shadow-sm">
        <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
          Showing {filteredOrders.length} items on Page {page}
        </div>
        ...
        <div className="flex items-center gap-4">
            ...
            className="p-2 border border-slate-100 rounded-xl text-slate-600 disabled:opacity-30 hover:bg-slate-50 transition-all"
            >
            <ChevronLeft size={20} />
            </button>

            <div className="flex items-center gap-2">
                <span className="h-8 w-8 rounded-lg bg-black text-white flex items-center justify-center text-xs font-black shadow-lg">
                    {page}
                </span>
                <span className="text-[10px] font-black text-slate-300 uppercase">of</span>
                <span className="h-8 w-8 rounded-lg bg-slate-50 text-slate-400 flex items-center justify-center text-xs font-black">
                    {pagination?.pages || 1}
                </span>
            </div>
...
const StatusCard = ({ label, value, icon, color }) => {
  ...
  return (
    <div className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm group hover:border-[#7e2827]/20 transition-all duration-300">
      <div className={`w-10 h-10 rounded-2xl ${themes[color]} flex items-center justify-center mb-4 shadow-xl`}>
        {icon}
      </div>
